...
public class MoveCamY : MonoBehaviour
{
    ...
    void Update() {
        if (move) {
            ...
            if (Input.GetMouseButton(0) && ((Input.GetAxis("Mouse X") > 0) || (Input.GetAxis("Mouse X") < 0) || (Input.GetAxis("Mouse Y") > 0) || (Input.GetAxis("Mouse Y") < 0)) && !restart) {
                ...
                if ((Input.GetAxis("Mouse Y") < 0) && (this.transform.position.y < 12f) && hit.collider == null) {

                    Vector3 pos = Camera.main.ScreenToViewportPoint(Input.mousePosition - dragOrigin);
                    Vector3 move = new Vector3(0, pos.y * (-dragSpeed), 0);
                    transform.Translate(move, Space.World);

                } else if ((Input.GetAxis("Mouse Y") > 0) && (this.transform.position.y > -12f) && hit.collider == null) {
